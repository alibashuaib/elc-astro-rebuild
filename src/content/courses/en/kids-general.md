---
title: Kids General English
summary: Fun, structured English learning for children aged 6–13, with age-appropriate lessons across a CEFR-aligned pathway.
category: kids
price: 1500
currency: SAR
durationWeeks: 4
hoursPerWeek: 8
hoursPerMonth: "34"
hoursPerDay: 2
scheduleDays: Sunday–Wednesday
classTimes:
  - label: Class time
    start: "5:00"
    end: "7:00"
    meridiem: PM
imageAlt: Children in an English class at ELC
faqs:
  - question: Who is the course for?
    answer: The program is designed for children aged 6 to 13. Boys and girls learn in dedicated sections supported by female trainers.
  - question: How is the program scheduled?
    answer: Each course runs for one month and totals 34 learning hours. Classes are two hours per day from Sunday to Wednesday; admissions can confirm the current start dates.
  - question: How is the learning pathway structured?
    answer: The program has 8 levels delivered across 12 courses and follows the CEFR international language framework.
  - question: Is Cambridge YLE preparation available?
    answer: After completing the relevant preparation course, learners may be ready to take a Cambridge Young Learners English (YLE) test.
draft: false
---

Kids General English is a fun, interactive program that helps young learners build strong foundations through age-appropriate lessons, engaging activities, and supportive classroom interaction.

Students develop vocabulary, sentence building, listening, speaking, reading, and writing through activities that encourage participation and confidence. The structured pathway helps each child progress clearly from the basics toward more independent use of English.

## Program at a glance

- Ages 6–13
- One month and 34 total learning hours per course
- Two hours per day, Sunday to Wednesday
- 8 CEFR-aligned levels delivered across 12 courses
- Dedicated sections for boys and girls, supported by female trainers
- SAR 1,500 per level

Contact ELC to confirm the next intake and placement guidance.
