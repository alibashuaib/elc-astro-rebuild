...
describe('router', () => {
  ...
  it('returns 401 for GET /api/admin/applications with no auth cookie', async () => {
    const req = new Request('http://x/api/admin/applications', { method: 'GET' });
    const res = await worker.fetch(req, env as any);
    expect(res.status).toBe(401);
  });

  it('returns 401 for PATCH /api/admin/applications/:id with no auth cookie', async () => {
    const req = new Request('http://x/api/admin/applications/app-1', {
      method: 'PATCH',
